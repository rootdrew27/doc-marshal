...
- **requires:** code_refs
- **frontmatter:** `type`, `updated`, `summary`, `code_refs`, `source` -- no other key
- **sections:** ## Prerequisites, ## Steps -- required, in this order, each with content; other sections allowed
- **title:** one H1, first
...
- **frontmatter:** `type`, `updated`, `summary`, `code_refs`, `source`, `status`, `supersedes`, `superseded_by` -- no other key
...
- **folder:** decisions/ at the docs root
...
- **sections:** ## Overview, ## Behavior, ## Validation -- required, in this order, each with content; other sections allowed
- **must be empty:** ## Open questions once status is done (the section itself is optional)
- **title:** one H1, first
...
- **requires:** no anchor
- **frontmatter:** `type`, `updated`, `summary`, `code_refs`, `source` -- no other key
- **filename:** NOMENCLATURE.md, required at the docs root
- **nesting:** a nested instance adds keys, never redefines an ancestor's
- **title:** one H1, first
