...
`enabled = false`, `[rules]` -- lands in 0.3 and replaces the body of `load_registry` without
    ...
    if marker.is_file():
        ...
        if data:
            raise DocMarshalError(
                ...
                f"{__version__} does not read -- configuration arrives in 0.3. Until then the file "
