...
An anchor field's `resolves` says what its entries may be -- URLs, paths under the docs tree,
...
from .git import Git
from .ontology import AnchorField, Profile
...
def describe_anchor(anchor: AnchorField, docs_tree: Path, repo_root: Path) -> str:
    """What the field's entries may be, as text for a message."""
    docs_prefix = rel_to(docs_tree, repo_root)
    words = {
        "url": "URLs",
...
def resolve_entry(entry: str, anchor: AnchorField, docs_tree: Path, git: Git, profile: Profile) -> str | None:
    """Why this entry fails the field's `resolves` kinds, or None when some kind accepts it.

    Every path in frontmatter is written from the repo root, never from the docs tree and never
    absolute -- one path convention for every field. A `docs-path` that resolves outside the docs
    tree is rejected on purpose: code belongs in a `repo-path` field, and accepting it here would
    let a note satisfy its anchor while staying off the repo-path anchor fields.
    ...
    admits no `.` or `..` segment, since the resolver and `drifted` would otherwise agree on a
    ...
    found = exists_exact(repo_root, resolved) and ("repo-path" in kinds or resolved.is_relative_to(docs_tree))
    if found:
        ...
        if not tracked:
            return (
                ...
                f"only -- commit it, or describe it in the body rather than anchoring to it: {entry}"
            )
        return None
    if "repo-path" in kinds:
        return f"{name} path does not exist (spelled exactly, from the repo root): {entry}"
    if not resolved.is_relative_to(docs_tree):
        docs_prefix = rel_to(docs_tree, repo_root)
        code_fields = " or ".join(profile.repo_path_fields) or "a repo-path field"
        url_part = "a URL or " if "url" in kinds else ""
        return (
            f"{name} must be {url_part}a path under {docs_prefix}/ -- written from the repo root, "
            f"and code paths belong in {code_fields} instead: {entry}"
        )
    return f"{name} path does not exist (spelled exactly, from the repo root): {entry}"
...
def check_anchor(
    ...
    if not isinstance(entries, list):
        report.error(path, f"'{anchor.name}' must be a list of {describe_anchor(anchor, docs_tree, git.repo_root)}")
        return
    for entry in entries:
        problem = resolve_entry(entry, anchor, docs_tree, git, profile)
